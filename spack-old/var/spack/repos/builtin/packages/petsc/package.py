...
class Petsc(Package, CudaPackage, ROCmPackage):
    ...
    def configure_options(self):
        ...
        if spec.compiler_flags["cflags"]:
            options += ["CFLAGS=%s" % " ".join(spec.compiler_flags["cflags"] + [self.compiler.cc_pic_flag])]
            if "+debug" not in spec:
                options += ["COPTFLAGS="]
        if spec.compiler_flags["fflags"]:
            options += ["FFLAGS=%s" % " ".join(spec.compiler_flags["fflags"] + [self.compiler.fc_pic_flag])]
            if "+debug" not in spec:
                options += ["FOPTFLAGS="]
        if spec.compiler_flags["cxxflags"]:
            options += ["CXXFLAGS=%s" % " ".join(spec.compiler_flags["cxxflags"] + [self.compiler.cxx_pic_flag])]
            if "+debug" not in spec:
                options += ["CXXOPTFLAGS="]
