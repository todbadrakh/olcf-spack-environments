...
class Cdo(AutotoolsPackage):
    ...
    variant("aec", default=False, description="Use libaec compression for GRIB1 instead of szip")
    ...
    conflicts(
        "+szip", when="+aec", msg="Must use only one of +szip or +aec"
    )
    conflicts(
        "+aec", when="+external-grib1 grib2=none", msg="The configuration does not support GRIB1"
    )
